...
======================
This file imports run_classifier_fhe_all_positions from fhe.llama3_mrpc, but
that symbol was REMOVED from the repo because prefill PPL is infeasible on the
decode-only FHE pipeline (one query position per forward pass).

Reviving PPL requires a multi-Q prefill rewrite (<=T_MODEL=8 positions/ct +
causal mask) — an algorithmic change, not a path fix.

PT-only WikiText-2 baseline (already measured):
